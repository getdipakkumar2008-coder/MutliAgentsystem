...
3. **Plan.** Propose a tier (T0-T3) and create the sealed plan with:
   `python "$CLAUDE_PROJECT_DIR/scripts/agentic.py" plan new --goal "<goal>" --tier <T0-T3> --file <path>[:add|modify|delete] ... --validate "<exact command>" ...`
   For T2/T3 also pass `--security "<item>"`. For richer plans, write `.agentic/plan.json` yourself and run `python "$CLAUDE_PROJECT_DIR/scripts/agentic.py" plan seal`.
